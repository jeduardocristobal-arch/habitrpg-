...
export default function HabitRPG() {
  const [state, setState] = useState(() => {
    ...
    if (saved.lastDate !== today && !saved.dead) {
      ...
      const newLog = [
        `📅 Nuevo día. HP -${hpLoss} | XP -${xpLoss}`,
        allDone && streak > 1 ? `🔥 ¡Racha de ${streak} días!` : null,
      ].filter(Boolean);
  ...
  const completeHabit = (habit) => {
    ...
    update(prev => ({
      ...
      completedToday: { ...prev.completedToday, [habit.id]: true },
      log: [`✅ ${habit.emoji} ${habit.name} +${earned}XP +${healAmt}HP`, ...prev.log].slice(0, 50),
    }));
    pushToast(`+${earned} XP ✨ +${healAmt} HP 💚`, "gain");
  };
                        ...
                        >
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            <div style={{ fontSize:11, color:"#475569", marginTop:2 }}>
                              {done ? "¡Completado! 🎉" : `+${habit.xp} XP · +5 HP al completar`}
                            </div>
                          </div>
                          {!done && (
                            <div style={{
              ...
              {state.streak > 0 && (
                <div style={{
                  ...
                  <div style={{ fontSize:11, color:"#78716c", marginTop:4 }}>
                    Bonus: +{Math.min(state.streak, MAX_STREAK_BONUS) * STREAK_BONUS_XP} XP extra por hábito completado
                  </div>
                </div>
              )}
            </div>
